package daon.be.agent.data.source.kis.quotation.program;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * KIS API: 프로그램매매 종합현황(일별)
 * TR ID: FHPPG04600001
 * URL: /uapi/domestic-stock/v1/quotations/comp-program-trade-daily
 * PRD-002 사용 여부: o
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public record KisFhppg04600001CompProgramTradeDailyRequest(
        @JsonProperty("FID_COND_MRKT_DIV_CODE") String fidCondMrktDivCode, // 시장 분류 코드
        @JsonProperty("FID_MRKT_CLS_CODE") String fidMrktClsCode, // 시장 구분 코드
        @JsonProperty("FID_INPUT_DATE_1") String fidInputDate1, // 검색시작일
        @JsonProperty("FID_INPUT_DATE_2") String fidInputDate2 // 검색종료일
) {
}
